...
class Adminmodel extends CI_Model
{
	...
	function get_item_by_id($id)
	{
		...
		$this->db->join('(SELECT '.$this->db->dbprefix('images').'.* FROM '.$this->db->dbprefix('images').' WHERE '.$this->db->dbprefix('images').'.module = '.$this->module_images_id.') AS projects_images', 'projects_images.item_id = projects.id', 'left'); 
		$this->db->order_by('projects.date', 'desc');
		$this->db->where(array('projects.id' => $id), 1);
